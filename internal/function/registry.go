...
func (fr *Registry) Register(fn Function) {
	fr.mu.Lock()
	defer fr.mu.Unlock()
	fr.functions[fn.ID] = fn
	go func() {
		_ = fr.saveToFile()
	}()
}

func (fr *Registry) Get(id string) (*Function, bool) {
	fn, exists := fr.functions[id]
	return &fn, exists
}

// GetByName Get function by name and runtime. These are typically unique
func (fr *Registry) GetByName(name string, runtime string) *Function {
	for _, f := range fr.functions {
		if f.Name == name && f.Runtime == runtime {
			return &f
		}
	}
	return nil
...
func (fr *Registry) Delete(fn *Function) {
	fr.mu.Lock()
	defer fr.mu.Unlock()
	delete(fr.functions, fn.ID)
	go func() {
		_ = fr.saveToFile()
	}()
}
